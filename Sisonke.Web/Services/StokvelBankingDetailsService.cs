...
public sealed class StokvelBankingDetailsService(IDbContextFactory<ApplicationDbContext> dbFactory)
{
    ...
    {
        Id = Guid.NewGuid(), StokvelId = stokvelId,
        BankName = model.BankName.Trim(), AccountHolderName = model.AccountHolderName.Trim(),
        AccountNumber = model.AccountNumber.Trim(), AccountType = model.AccountType,
        BranchCode = NullIfWhiteSpace(model.BranchCode), BranchName = NullIfWhiteSpace(model.BranchName),
        PaymentReferenceFormat = NullIfWhiteSpace(model.PaymentReferenceFormat), Notes = NullIfWhiteSpace(model.Notes),
        IsPrimary = true, IsActive = true, CreatedAt = now, CreatedBy = currentUserId
    };
