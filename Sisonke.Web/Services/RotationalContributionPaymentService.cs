    ...
    ILogger<RotationalContributionPaymentService> logger)
{
    ...
    public async Task<List<RotationalContributionPayment>> GetPaymentsForMemberAsync(Guid stokvelId, Guid memberId, int take = 6)
    {
        await using var context = await dbFactory.CreateDbContextAsync();
        ...
        return await context.RotationalContributionPayments
            ...
            .Where(p =>
                ...
                p.MemberId == memberId &&
                p.IsActive)
            .OrderByDescending(p => p.Cycle.CycleNumber)
            .ThenByDescending(p => p.Cycle.CycleStartDate)
        ...
        Guid paymentId, ConfirmPaymentRequest request, string currentUserId)
    {
        ...
        if (stokvel.Archetype != StokvelArchetype.BurialSociety)
        {
            ...
            if (surplusDelta < 0)
            {
                ...
                if (wallet is null || wallet.AvailableBalance < Math.Abs(surplusDelta))
                    return PaymentConfirmResult.Failed(["Cannot reduce this overpayment because the surplus wallet funds have already been withdrawn."]);
            }

            await ApplyContributionOverpaymentAsync(
                context,
                payment.StokvelId,
                payment.MemberId,
                payment.Id,
    ...
    private static async Task<decimal> GetContributionOverpaymentBalanceAsync(ApplicationDbContext context, Guid paymentId)
        => await context.MemberSurplusWalletTransactions
            .Where(entry => entry.SourceReferenceId == paymentId &&
                            entry.SourceType == WalletTransactionSourceType.ContributionOverpayment)
            .SumAsync(entry => entry.TransactionType == WalletTransactionType.Credit
                ? entry.Amount
                : -entry.Amount);

    private static async Task ApplyContributionOverpaymentAsync(
        ApplicationDbContext context,
        Guid stokvelId,
        Guid memberId,
        Guid paymentId,
        decimal surplusDelta,
        ...
        DateTime now)
    {
        ...
        var wallet = await context.MemberSurplusWallets.FirstOrDefaultAsync(item =>
            item.StokvelId == stokvelId && item.MemberId == memberId && item.IsActive);
        if (wallet is null)
        {
        ...
        context.MemberSurplusWalletTransactions.Add(new MemberSurplusWalletTransaction
        {
            ...
            WalletId = wallet.Id,
            MemberId = memberId,
            TransactionType = surplusDelta > 0 ? WalletTransactionType.Credit : WalletTransactionType.Debit,
            Amount = Math.Abs(surplusDelta),
            ...
            Description = surplusDelta > 0 ? "Contribution overpayment" : "Contribution overpayment adjustment",
            CreatedAt = now,
            CreatedBy = currentUserId
        });
    }
