    ...
    ILogger<LoansWalletService> logger)
{
        ...
        Guid stokvelId, StokvelLoanConfiguration model, string currentUserId)
    {
        ...
        {
            Id = Guid.NewGuid(), StokvelId = stokvelId, LoansEnabled = model.LoansEnabled,
            MinLoanAmount = model.MinLoanAmount, MaxLoanAmount = model.MaxLoanAmount,
            MaxRepaymentMonths = model.MaxRepaymentMonths, DefaultRepaymentMonths = model.DefaultRepaymentMonths,
            LoanInterestType = model.LoanInterestType, LoanInterestRate = model.LoanInterestRate,
            LateRepaymentFineType = model.LateRepaymentFineType, LateRepaymentFineAmount = model.LateRepaymentFineAmount,
        ...
        Guid stokvelId, string currentUserId, decimal amount, int repaymentMonths, string reason)
    {
        ...
        var now = DateTime.UtcNow;
        context.MemberLoans.Add(new MemberLoan
        {
            Id = Guid.NewGuid(), StokvelId = stokvelId, MemberId = access.Member.Id,
            RequestedAmount = amount, RepaymentMonths = repaymentMonths, RequestReason = reason.Trim(),
            LoanStatus = MemberLoanStatus.PendingApproval, RequestedAt = now, RequestedBy = currentUserId,
            IsActive = true, CreatedAt = now, CreatedBy = currentUserId
        });
        await context.SaveChangesAsync();
    ...
    public async Task<FinanceOperationResult> ApproveLoanAsync(Guid loanId, string currentUserId, decimal approvedAmount, int months)
    {
        ...
        var now = DateTime.UtcNow;
        loan.ApprovedAmount = approvedAmount; loan.RepaymentMonths = months; loan.TotalRepayableAmount = total;
        loan.MonthlyRepaymentAmount = Math.Round(total / months, 2); loan.OutstandingBalance = total;
        loan.LoanStatus = MemberLoanStatus.DisbursementPending; loan.ApprovedAt = now;
        loan.ApprovedByChairpersonId = currentUserId; loan.UpdatedAt = now; loan.UpdatedBy = currentUserId;
    ...
    public async Task<FinanceOperationResult> ConfirmDisbursementAsync(Guid loanId, string currentUserId, PaymentMethod? method, string? reference, DateTime? date, string? notes)
    {
        ...
        if (loan.LoanStatus != MemberLoanStatus.DisbursementPending) return FinanceOperationResult.Failed("Loan is not awaiting disbursement.");
        var now = DateTime.UtcNow; var dueStart = date.Value.AddMonths(1).Date;
        loan.LoanStatus = MemberLoanStatus.Active; loan.DisbursementMethod = method; loan.DisbursementReference = reference.Trim();
        loan.DisbursedAt = date; loan.DisbursedByTreasurerId = currentUserId; loan.DueStartDate = dueStart;
        loan.ExpectedFinalPaymentDate = dueStart.AddMonths(loan.RepaymentMonths - 1); loan.Notes = Trim(notes); loan.UpdatedAt = now; loan.UpdatedBy = currentUserId;
        for (var i = 0; i < loan.RepaymentMonths; i++)
        {
            var expected = i == loan.RepaymentMonths - 1 ? loan.TotalRepayableAmount - loan.MonthlyRepaymentAmount * (loan.RepaymentMonths - 1) : loan.MonthlyRepaymentAmount;
            context.MemberLoanRepayments.Add(new MemberLoanRepayment { Id = Guid.NewGuid(), StokvelId = loan.StokvelId, LoanId = loan.Id, MemberId = loan.MemberId, ExpectedAmount = expected, DueDate = dueStart.AddMonths(i), PaymentStatus = LoanRepaymentStatus.Pending, IsActive = true, CreatedAt = now, CreatedBy = currentUserId });
        }
    ...
    public async Task<FinanceOperationResult> ConfirmRepaymentAsync(Guid repaymentId, string currentUserId, decimal paidAmount, DateTime? paymentDate, PaymentMethod? method, string? reference, bool waive, bool applyFine, string? notes)
    {
        ...
        var settled = schedule.Sum(x => x.PaymentStatus == LoanRepaymentStatus.Waived ? x.ExpectedAmount : x.PaidAmount);
    ...
    public async Task CreditContributionOverpaymentAsync(Guid stokvelId, Guid memberId, Guid paymentId, decimal amount, string currentUserId)
    {
        ...
        var exists = await context.MemberSurplusWalletTransactions.AnyAsync(x => x.SourceReferenceId == paymentId && x.SourceType == WalletTransactionSourceType.ContributionOverpayment && x.TransactionType == WalletTransactionType.Credit);
        if (exists) return;
        ...
        wallet.AvailableBalance += amount; wallet.TotalCredits += amount; wallet.UpdatedAt = DateTime.UtcNow; wallet.UpdatedBy = currentUserId;
        context.MemberSurplusWalletTransactions.Add(new() { Id = Guid.NewGuid(), StokvelId = stokvelId, WalletId = wallet.Id, MemberId = memberId, TransactionType = WalletTransactionType.Credit, Amount = amount, BalanceAfterTransaction = wallet.AvailableBalance, SourceType = WalletTransactionSourceType.ContributionOverpayment, SourceReferenceId = paymentId, Description = "Contribution overpayment", CreatedAt = DateTime.UtcNow, CreatedBy = currentUserId });
        await context.SaveChangesAsync(); await tx.CommitAsync();
        logger.LogInformation("Surplus wallet credited from contribution payment {PaymentId}", paymentId);
    }
